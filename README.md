...
To generate speech from an image, you can use the provided Gradio interface or run the inference script.
...
gradio_interface = gr.Interface(
    fn=imagecraft_interface,  # Function that takes an image and returns audio
    inputs=[gr.Image(type="filepath", label="Upload an image")],
    outputs=[gr.Audio(label="Generated Speech")],
    title="ImageCraft Assistant",
    description="Upload an image and listen to the speech generated by the model.",
...
```bash
python inference.py --input_image path/to/image.jpg --output_audio path/to/output.wav
```
...
```python
python train.py --config config.yaml --checkpoint_dir checkpoints/
```
